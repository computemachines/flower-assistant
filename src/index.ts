...
let resourcesPath;
if (isDevMode) {
    resourcesPath = path.join(appPath, "node_modules", "electron-flowno-bridge", "resources");
} else {
...
console.log(
    `[Electron Main] PythonRunner started with resources path: ${resourcesPath}`,
);
...
const intervalId = setInterval(() => {
    console.log(
        "[Electron Main] JS: Python runner is running in a separate thread...",
    );
}, 1000);
